...
export default function RestaurantOperationalSettingsCard() {
  const { selectedRestaurant: restaurant, updateSelectedRestaurant } =
    useRestaurantStore();
  ...
  const STATUS_OPTIONS: { label: string; value: RestaurantStatus }[] = [
    ...
    {
      label: t("fields.status.options.DISABLE_DELIVERY"),
      value: "DISABLE_DELIVERY",
    },
    {
      label: t("fields.status.options.DISABLE_PICKUP"),
      value: "DISABLE_PICKUP",
    },
  ...
  const hasFeeChanged =
    Number.isFinite(normalizedNextFee) && normalizedNextFee !== initialFee;
  ...
  return (
    <Card className="bg-white rounded-xl grid lg:grid-cols-4 md:grid-cols-2  px-5 py-4 gap-3 overflow-hidden relative">
      {saving && (
        <div className="flex items-center justify-center flex-col gap-1 absolute inset-0 bg-white/90">
          ...
          <p className="text-sm text-black/50">{t("saving")}</p>
        </div>
      )}
      ...
      {/* Delivery Fee */}
      <div className="space-y-2 ">
        <Label htmlFor="delivery_fee">{t("fields.deliveryFee.label")}</Label>
      ...
      {/* Tax Fee */}
      <div className="space-y-2 ">
        <Label htmlFor="tax_percentage">{t("fields.tax.label")}</Label>
        <div className="flex gap-2">
          <Input
            id="tax_percentage"
            type="number"
      ...
      <div className="w-full h-full flex items-end">
        <Button
          ...
          disabled={
            saving ||
            !hasChanges ||
            (hasFeeChanged && feeIsInvalid) ||
            (hasTaxChanged && taxIsInvalid)
          }
