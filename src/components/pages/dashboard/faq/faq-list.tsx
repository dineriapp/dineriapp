...
export default function FaqList({
  ...
  const t = useTranslations("faqs.faqCategoriesComponent.faqList");

  if (!faqs || faqs.length === 0) {
    return (
      <div className="py-8 text-center">
        <HelpCircle className="mx-auto mb-2 h-8 w-8 text-gray-400" />
        <p className="text-slate-500 mb-4">{t("noFaqsMessage")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      {faqs.map((faq, index) => (
        <div
          key={faq.id}
          className="flex items-start gap-3 rounded-lg border bg-white/90 p-4 backdrop-blur-sm transition-all hover:shadow-md hover:scale-[1.01] max-md:flex-col"
        >
          {/* FAQ Text */}
          ...
          {/* Action buttons */}
          <div className="flex flex-shrink-0 items-center max-md:justify-center max-md:w-full gap-1">
            {/* Edit */}
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              ...
              <span className="sr-only">{t("editFaq")}</span>
            </Button>
            ...
            <AlertDialog>
              <AlertDialogTrigger asChild>
                ...
                >
                  ...
                  <span className="sr-only">{t("deleteFaq")}</span>
                </Button>
              </AlertDialogTrigger>
              <AlertDialogContent>
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    {t("deleteFaqDescription", { question: faq.question })}
                  </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
