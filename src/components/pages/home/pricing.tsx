        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {Object.entries(STRIPE_PLANS).map(([key, plan]) => {
            ...
            return (
              <div
                key={planKey}
                className={`relative rounded-xl p-7 transition-all duration-200 ${
                  isPopular
              ...
              >
                ...
                <div className="my-4">
                  ...
                  <span
                    className={`text-sm pl-1 ${
                      isPopular ? "text-white/80" : "text-main-text"
                    }`}
                  >
                    /month
                  </span>
                ...
                <p
                  className={`mb-6 ${
                    isPopular ? "text-white/90" : "text-main-text"
                  }`}
                >
                  {plan.description}
                </p>
                ...
                <ul className="mb-8 space-y-3">
                  {plan.features.map((feature, i) => (
                    <li key={i} className="flex items-center gap-2">
                      <Check
                        className={`h-5 w-5 ${
                          isPopular ? "text-white" : "text-main-action"
                        }`}
                      />
                      <span>{feature}</span>
                    </li>
                ...
                <Button
                  ...
                  className={`w-full h-[44px] cursor-pointer ${
                    isPopular
