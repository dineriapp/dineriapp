...
import { useOrders, useOrderStats, useUpdateOrderStatus, useRefreshOrders, useExportOrders, OrderWithItems } from "@/lib/order-queries"
...
export default function OrdersPage() {
    ...
    const { data: orders = [], isLoading, error, isRefetching } = useOrders(restaurant?.id)
    const { data: stats, isLoading: statsLoading } = useOrderStats(restaurant?.id)
    ...
    const filteredOrders = useMemo(() => {
        return orders.filter((order) => {
            ...
            return matchesSearch && matchesStatus && matchesPaymentStatus && matchesOrderType && matchesDate
        })
    }, [orders, searchTerm, statusFilter, paymentStatusFilter, orderTypeFilter, dateFilter])
        ...
        <div className="space-y-6 max-w-[1200px] mx-auto w-full py-8">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                    <Button variant="outline" onClick={handleRefresh} disabled={isRefetching || refreshOrders.isPending}>
                        <RefreshCw className={`h-4 w-4 mr-2 ${isRefetching ? "animate-spin" : ""}`} />
                        Refresh
                    </Button>
                    <Button variant="outline" onClick={handleExport} disabled={exportOrders.isPending}>
                    ...
                    <>
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground">{stats.today_orders} today</p>
                            </CardContent>
                        </Card>
                        ...
                        <Card>
                            ...
                            <CardContent>
                                ...
                                <p className="text-xs text-muted-foreground">${stats.today_revenue.toFixed(2)} today</p>
                            </CardContent>
                        </Card>
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>Manage your restaurant orders and update their status</CardDescription>
                </CardHeader>
                <CardContent>
                                ...
                                {isLoading ? (
                                    ...
                                    filteredOrders.map((order) => (
                                        <TableRow key={order.id}>
                                            <TableCell className="font-mono text-sm">{order.order_number.split("-").slice(-1)[0]}</TableCell>
                                            <TableCell>
                                                <div>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {getNextStatus(order.status) && order.status !== "cancelled" && (
                                                        ...
                                                        >
                                                            {updateOrderStatus.isPending ? "Updating..." : getStatusAction(order.status)}
                                                        </Button>
                                                    )}
                    ...
                    <CardContent className="space-y-3">
                        ...
                        {order.estimated_ready_time && (
                            <div>
                                ...
                                <p className="text-sm font-medium">{new Date(order.estimated_ready_time).toLocaleString()}</p>
                            </div>
                        )}
                    </CardContent>
                </Card>
                            ...
                            <div key={item.id} className="flex justify-between items-start border-b pb-3 last:border-b-0">
                                <div className="flex-1">
                                    ...
                                    {item.special_requests && (
                                        <p className="text-sm text-blue-600 mt-1">Special: {item.special_requests}</p>
                                    )}
