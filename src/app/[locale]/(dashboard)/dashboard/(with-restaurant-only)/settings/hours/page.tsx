...
export default function HoursPage() {
  ...
  const [formData, setFormData] =
    useState<OpeningHoursFormData>(defaultFormData);
  ...
  console.log({
    selectedTimeZone,
    initial: selectedRestaurant?.timezone || "",
  });
  ...
  const updateDayHours = (
    dayKey: keyof OpeningHoursFormData,
  ...
  const saveOpeningHours = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/restaurants/${selectedRestaurant.id}/hours`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || t("form.errors.updateError_description"),
        );
      }
  ...
  if (loading || !selectedRestaurant) {
    return <LoadingUI text={t("loading.text")} />;
  }
        ...
        <CardContent className="pt-4">
            ...
            <Popover open={open} onOpenChange={setOpen}>
              <PopoverTrigger asChild>
                ...
                >
                  {selectedTimeZone
                    ? selectedTimeZone
                    : t("form.placeholders.selectTimezone")}
                  <ChevronsUpDown className="ml-2 h-4 w-4 opacity-50" />
                </Button>
              </PopoverTrigger>

              <PopoverContent className=" min-w-full! w-[var(--radix-popover-trigger-width)] p-2 ">
                  ...
                  {allTimezones
                    ...
                    .map((tz) => (
                      <div
                        ...
                        className="flex items-center justify-between px-2 py-2 text-sm rounded-md cursor-pointer hover:bg-gray-100"
                      >
                        <span>{tz}</span>
                        {selectedTimeZone === tz && (
          ...
          <div className="space-y-6">
              ...
              <div key={dayKey} className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() =>
                      toggleDayClosed(dayKey as keyof OpeningHoursFormData)
                    }
                    className={`h-[38px] rounded-full px-4 text-xs font-poppins cursor-pointer ${dayData.closed ? "text-white bg-main-blue hover:bg-main-blue/70 hover:text-white" : "bg-main-green text-white hover:bg-main-green/70 hover:text-white"}`}
                  >
                    {dayData.closed
                      ? t("form.buttons.closed")
                      : t("form.buttons.open")}
                  </Button>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-2 w-full">
                      ...
                      <Select
                        ...
                        onValueChange={(value) =>
                          updateDayHours(
                            dayKey as keyof OpeningHoursFormData,
                            "open",
                            value,
                          )
                        }
                      >
                        <SelectTrigger className="focus:border-emerald-500 focus:ring-emerald-500 w-full">
                          <SelectValue
                            placeholder={t(
                              "form.placeholders.selectOpeningTime",
                            )}
                          />
                        </SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-2 w-full">
                      ...
                      >
                        <SelectTrigger className="focus:border-emerald-500 w-full focus:ring-emerald-500">
                          <SelectValue
                            placeholder={t(
                              "form.placeholders.selectClosingTime",
                            )}
                          />
                        </SelectTrigger>
                        <SelectContent>
            ...
            <div className="mt-6 p-4 bg-slate-50 rounded-lg">
              <p className="text-sm text-slate-600">{t("form.tips.general")}</p>
            </div>
          </div>
        </CardContent>
      </Card>
