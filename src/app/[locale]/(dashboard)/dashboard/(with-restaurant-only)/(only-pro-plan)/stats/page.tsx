...
export default function StatsPage() {
  ...
  const {
    ...
    error,
    dataUpdatedAt,
  } = useStats(selectedRestaurant?.id);
  ...
  if (error) {
    return (
      <ErrorMessage title={t("error.title")} message={t("error.message")} />
    );
  }

  if (!stats) {
    return (
      <ErrorMessage title={t("noData.title")} message={t("message.message")} />
    );
  }
  ...
  const overviewCards = [
    {
      ...
      extra:
        stats.overview.recentLinkViews > 0
          ? o("totalLinkViews.extra.thisWeek", {
              count: stats.overview.recentLinkViews,
            })
          : null,
    ...
    },
    {
      ...
      extra:
        stats.overview.totalViews > 0
          ? o("uniqueLinkVisitors.extra.uniquePercent", {
              percent: Math.round(
                (stats.overview.totalUniqueViews / stats.overview.totalViews) *
                  100,
              ),
            })
    ...
    },
    {
      ...
      extra:
        stats.overview.recentPageViews > 0
          ? o("pageViews.extra.thisWeek", {
              count: stats.overview.recentPageViews,
            })
          : null,
      extraClass: "text-green-600",
    },
    {
      ...
      extra:
        stats.overview.totalPageViews > 0
          ? o("uniquePageVisitors.extra.uniquePercent", {
              percent: Math.round(
                (stats.overview.uniquePageViews /
                  stats.overview.totalPageViews) *
                  100,
              ),
            })
      ...
      <div className="mb-8 flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-500">
            {t("page.lastUpdated")} {lastUpdated}
          </span>
        </div>
      </div>
        ...
        <Card className="mb-8 border-gray-200 shadow-sm">
          ...
          <CardContent className="py-8">
            <div className="text-center text-gray-500">
              ...
              <p>{t("linkPerformance.noLinks.message")}</p>
            </div>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent>
            <div className="md:hidden block w-full">
              {stats.linkStats.map((stat) => (
                <div key={stat.id} className="border-b border-gray-100">
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-gray-900 mb-2">
                      {device.device === "Mobile" &&
                        t("deviceBreakdown.devices.mobile")}
                      {device.device === "Desktop" &&
                        t("deviceBreakdown.devices.desktop")}
                      {device.device === "Tablet" &&
                        t("deviceBreakdown.devices.tablet")}
                    </h3>
                    <div className="text-3xl font-bold text-gray-900 mb-1">
                      {device.count}
                    </div>
                    <div className="text-sm text-gray-500">
                      {t("deviceBreakdown.devices.percentageOfTotal", {
