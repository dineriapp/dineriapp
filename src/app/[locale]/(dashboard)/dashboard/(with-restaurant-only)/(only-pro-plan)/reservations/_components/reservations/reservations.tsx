...
import {
  ...
  useReservations,
  useSendReservationReminderEmail,
...
export default function ReservationsPage() {
  ...
  const views = [
    { key: "list", label: t("view.list"), icon: <List className="w-4 h-4" /> },
    {
      ...
      icon: <MdOutlineViewTimeline className="w-4 h-4" />,
    },
  ];
  ...
  const { data: reservations = [], isLoading: reservationsLoading } =
    useReservations(queryParams, shouldFetch);
    ...
    <>
      <div className="space-y-4 relative">
        ...
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
          <div className="flex gap-2 w-full md:w-auto max-md:flex-col">
            <Input
              placeholder={t("searchPlaceholder")}
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className=" bg-white h-11! md:flex-1"
            />
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              <SelectTrigger className="md:w-40 h-11! w-full bg-white">
            ...
            <Select value={dateFilter} onValueChange={setDateFilter}>
              <SelectTrigger className="md:w-40 h-11! w-full bg-white border border-border/50 shadow-sm hover:border-primary/30 transition-colors">
            ...
            {dateFilter === "CUSTOM" && (
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="md:w-44 w-full h-12 justify-start text-left font-normal"
                  >
