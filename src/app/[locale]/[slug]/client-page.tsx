...
export default function ClientPage({
  ...
  useEffect(() => {
    ...
    restaurant.slug,
    restaurant.welcome_popup_enabled,
    restaurant.welcome_popup_delay,
  ]);
  ...
  const getFilteredMenuItems = () => {
    ...
    const category = restaurant.menuCategories
      .filter((c) => c.show_in_quick_menu)
  ...
  const bookingDisabled =
    restaurant?.reservation_settings?.settings?.restaurantSettings
  ...
  const hasFaqsItems = restaurant.faqCategories?.some(
  ...
  const hasCustomLinks = restaurant.links.length > 0;

  const hasMenu =
    restaurant.menuCategories.length > 0 && !isMenuPage && hasMenuItems;

  const hasEvents = restaurant.events.length > 0;
              ...
              {!bookingDisabled &&
                restaurant?.user?.subscription_plan !== "basic" && (
                  <motion.a
                    ...
                    className={`group flex items-center justify-center  text-center  w-full  py-5 transition-all hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden ${restaurant?.button_icons_show ? "px-14 sm:px-16" : "px-4"} ${
                  ...
                  >
                    ...
                    <span
                      className={`relative w-full text-[26px] ${
              ...
              {restaurant.menuCategories.length > 0 &&
                ...
                hasMenuItems && (
                  <motion.button
                    ...
                    className={`group flex items-center justify-center  text-center ${restaurant?.button_icons_show ? "px-14 sm:px-16" : "px-4"} w-full  py-5 transition-all hover:scale-[1.02] active:scale-[0.98] relative overflow-hidden ${
                      restaurant.button_style === "pill"
                        ? "rounded-full"
                        : restaurant.button_style === "square"
                          ? "rounded-md"
                ...
                <DialogHeader>
                  ...
                  >
                    {restaurant.events.length === 1 ? (
                      <>
                        {e("description_singular", {
                          count: restaurant.events.length,
                        })}
                      </>
                    ) : (
                      <>
                        {e("description_plural", {
                          count: restaurant.events.length,
                        })}
                      </>
                    )}
                ...
                <div className="space-y-6 py-4">
                  {restaurant.events.map((event) => (
                    <div
                      ...
                      style={{
                        borderColor: restaurant.headings_text_color || "white",
                      }}
                    >
                      <h3
