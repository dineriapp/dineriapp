...
import { DynamicRule, OverridesSettings, SettingsState, TimeSlotOverride } from "@/app/[locale]/(dashboard)/dashboard/(with-restaurant-only)/reservations/_components/settings/types";
...
const BookingInterface = ({ restaurant }: { restaurant: Restaurant & { reservation_settings: { settings: SettingsState }, Reservation_policy: ReservationPolicy | null } }) => {
    ...
    const [canCreateReservation, setCanCreateReservation] = useState<boolean | null>(true);
                            ...
                            <Card className="p-5 shadow-lg relative overflow-hidden">
                                ...
                                <div className="space-y-3">
                                    ...
                                    </div>
                                    {
                                        ...
                                        <div className="px-4 py-3 bg-slate-50 border border-slate-200 text-slate-700 rounded-md">
                                            ...
                                            <ReservationDialog restaurantId={restaurant.id} timezone={restaurant.timezone || ""}>
                                                <button
                                                    className="mt-2 cursor-pointer text-xs px-2.5 py-1.5 rounded-md font-medium bg-slate-800 text-white hover:bg-slate-900 transition"
                                                >
                                    ...
                                    <div className="space-y-2">
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <div className="space-y-2">
                                                <Label htmlFor="party-size" className="text-sm font-medium text-gray-700">
                                                    Party Size *
                                                </Label>
                                        ...
                                        {date && selectedTime && (
                                            <div className="mt-4">
                                                <DailyCapacityWidget
                                                    restaurantId={restaurant?.id}
                                                    date={getUTCFromLocalDateTime(date.toISOString().split("T")[0], selectedTime, restaurant.timezone || "Europe/Rome")}
                                                    partySize={Number(partySize)}
                                                    onLoadingChange={(isLoading) => setLoading(isLoading)}
                                                    onAvailabilityChange={(value) => setCanCreateReservation(value)}
                                                />
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="space-y-3 relative">
                                    ...
                                    {(date || selectedTime || partySize !== "2") && (
                                        <div className="space-y-3 pt-4 border-t">
                                            <h4 className="font-semibold text-gray-900">Your Reservation</h4>
                                            ...
                                            {area !== "none" && (
                                                <div className="flex justify-between items-center">
                                                    <span className="text-sm text-gray-600">Preferred Area:</span>
                                                    <span className="text-sm font-medium text-gray-900">
                                    ...
                                    {displayDepositAmount > 0 && (
                                        <div className="pt-4 border-t">
                                            ...
                                            <p className="text-xs text-gray-500">
                                                This deposit will be applied to your final bill and is refundable according to our cancellation policy.
                                            </p>
                                        </div>
                                    )}
                                                ...
                                                <div className="pt-4 border-t">
                                                    ...
                                                    <div className="space-y-2 text-sm text-gray-600">
                                                        {settings.deposit_settings.cancellationPolicies
                                                            ...
                                                            .map((policy: any) => (
                                                                <div key={`${Math.random()}-${Math.random()}`} className="flex items-start gap-2">
                                    ...
                                    <button
                                        type="submit"
                                        disabled={isSubmitting || !date || !selectedTime || !name || !email || isTimeBlocked || !canCreateReservation || loading || !phoneRegex.test(phone)}
                                    ...
                                    >
                                        {isSubmitting ? (
                                            <span className="flex items-center justify-center">
                                                ...
                                                {displayDepositAmount > 0 ? 'Processing...' : 'Creating Reservation...'}
                                            </span>
                                        ) : isTimeBlocked ? (
                                            "Time Slot Unavailable"
                                        ) : (
                                            `Complete Reservation ${displayDepositAmount > 0 ? '& Pay Deposit' : ''}`
                                        )}
                                    </button>
                                    {/* Help Text */}
                                    <div className="pt-4 border-t">
                                        <p className="text-xs text-gray-500">
                                            Need to make changes? Contact us at {restaurant.phone || 'the restaurant directly'}.
                                        </p>
                                    </div>
